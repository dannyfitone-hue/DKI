...
export default function AccountCRM(){
  ...
  const [form,setForm]=useState({name:'',property_name:'',address:'',manager_name:'',phone:'',email:'',units:'',stage:'NEW PROSPECT',account_owner:'Daniel'});
  async function load(){ const r=await fetch('/api/accounts',{cache:'no-store'}); setAccounts(await r.json()); }
  useEffect(()=>{load()},[]);
  async function create(e){e.preventDefault(); const r=await fetch('/api/accounts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)}); if(r.ok){setOpen(false);setForm({name:'',property_name:'',address:'',manager_name:'',phone:'',email:'',units:'',stage:'NEW PROSPECT',account_owner:'Daniel'});load();}}
  ...
  return <>
    ...
    <div className="accountGrid">{filtered.map(a=><article className="accountCard" key={a.id}>
      ...
      <h3>{a.name}</h3><div className="propertyTitle">{a.property_name||'Property not added yet'}</div>
    ...
    {open&&<div className="modalBackdrop"><div className="modal wideModal"><div className="row"><div><div className="eyebrow">NEW PROPERTY MANAGEMENT RELATIONSHIP</div><h2>Add Account</h2></div><button className="smallBtn" onClick={()=>setOpen(false)}>Close</button></div>
      ...
      <button className="primaryBtn">Create Account</button></form>
    </div></div>}
  </>;
}